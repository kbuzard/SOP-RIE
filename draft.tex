...
The only strategic difference between the first-moving legislature and the second moving legislature is that the first mover faces uncertainty about whether the other legislature will break the trade agreement. In constructing the break probability for the foreign legislature $b^*(e_b^*,\bta)$, Expression~\ref{eq:lwcg} becomes
		...
		\]
	This inequality determining when the foreign legislature prefers to break the trade agreement can be rearranged as 
		\begin{equation}
	...
	This is equivalent to 	
		\[
			W_{ML}^*(\btn,\ga(e_b^*,\ve_b^*)) > W_{ML}^*\left(\bta,\ga(e_b^*,\ve_b^*)\right)
		\]
	Then given symmetry, the foreign legislature's incentives to break the trade agreement are identical to the home legislature's incentives to break the trade agreement given in Expression~\ref{eq:lwcg}. We can therefore state propositions analogous to Result~\ref{res:bincC} and Lemma~\ref{res:bcomb} for the foreign legislature. The proofs are identical modulo symmetry.

\begin{result}
  The probability that the foreign legislature breaks a trade agreement is increasing and concave in $e_b^*$.
  \label{res:bincCforeign}
\end{result}

\begin{lemma}
	Holding lobbying effort constant, the probability the foreign legislature breaks a trade agreement is weakly decreasing in $\bta$ (i.e. $\frac{\partial b^*(e^*,\bta)}{\partial  \tau^a} + \frac{\partial b^*(e^*,\bta)}{\partial  \tau^{*a}} \leq 0$).
	\label{res:bcombforeign}
\end{lemma}

\subsubsection{First Mover Lobby}
The lobby in the foreign country likewise has a more complex decision. The second-moving lobby's objective function in Expression~\ref{eq:lobby} becomes
  \begin{multline*}
...
\begin{result}
  When the trade agreement remains in force with positive probability, home lobbying effort is weakly decreasing in trade agreement tariffs.
  \label{res:lobbyforeign}
\end{result}
...
\vskip.4in
\noindent \textbf{\hypertarget{Pr_lobbyforeign}{Proof of Result~\ref{res:lobbyforeign}}}:\\
Proof is via the Implicit Function Theorem using the lobby's first order condition, Equation~\ref{eq:lobbyfoc}, referred to here as $FOC_L$.
  \[
		\frac{\partial e_b}{\partial \tau^a} + \frac{\partial e_b}{\partial \tau^{*a}} = - \left[ \frac{\frac{\partial FOC_L}{\partial \tau^a}}{\frac{\partial FOC_L}{\partial e_b}} + \frac{\frac{\partial FOC_L}{\partial \tau^{*a}}}{\frac{\partial FOC_L}{\partial e_b}} \right] = \frac{ \frac{\partial b}{\partial e_b} \frac{\partial \pi(\bta)}{\partial \tau^a} + \frac{\partial b}{\partial e_b} \frac{\partial \pi(\bta)}{\partial \tau^{*a}} - \left\{\frac{\partial^2 b}{\partial e_b \partial \tau^a} + \frac{\partial^2 b}{\partial e_b \partial \tau^{*a}}   \right\}\left[ \pi(\tau^n) - e_n - \pi(\tau^a) \right]}{\frac{\partial^2 b}{\partial e_b{}^2} \left[ \pi(\tau^n) - e_n - \pi(\tau^a) \right]}
	\]
