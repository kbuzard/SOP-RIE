...
\begin{equation}
  ...
  \label{eq:lv}
\end{equation}
where $\pi(\cdot)$ is the current-period profit and $\tau^a \ (\tau^\mathit{n})$ is the home country's tariff on the import good under a trade agreement (war). 

In the first stage, the executives choose the trade agreement tariffs. I model this choice via the Nash bargaining solution with a disagreement point of executive welfare in the non-cooperative game between the legislatures. In this symmetric environment, the executives maximize their expected joint payoffs:
\begin{multline}
...
The lower world price for home's export good has a negative effect on producer surplus that is larger than the positive effect on consumer surplus. Thus the net effect of an increase in foreign trade-agreement tariffs on both home legislative and home total social welfare is negative, leading to an increased probability that the trade agreement will be broken.
