...
\begin{equation}
  ...
  \label{eq:lv}
\end{equation}
where $\pi(\cdot)$ is the current-period profit \textbf{and the subscript $X$ has been dropped for notational convenience}. 
...
\noindent The price of good $Y$ is decreasing in $\tau^{*a}$, so consumer surplus is increasing and \textbf{profits} are decreasing in $\tau^{*a}$. Because $Y$ is being exported, the decrease in \textbf{profits} is larger than the increase in consumer surplus, making the numerator negative. Since \textbf{profits} in the denominator are increasing in $\tau$, the expression in Equation~\ref{appex:astar} is negative for all $\tau^{*a}$.  $\hfill\blacksquare$
...
Because total social welfare is maximized at $\tau^a = \tau^{*a} = 0$, $W(\btn)-W(\bta)$ is always negative, whereas \textbf{profits} are increasing in $\tau^a$, so the second summand is positive everywhere. With a positive denominator, we thus have that the derivative is positive on $[0,\tau^O]$.
