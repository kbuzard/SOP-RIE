...
In the non-unitary government model, these estimates and the earlier-cited stylized facts become consistent with each other. The ``case of the missing contributions,'' as Gawande and Hoekman (2006) refer to it, turns out to be well explained by an equilibrium phenomenon arising from the agenda-setting power of the executive branch. In particular, \textbf{in this model, there is no inconsistency for the approximately 15$\%$ of sectors that receive protection but apparently put forth no lobbying effort. In this framework, even a welfare-maximizing executive and lobbying effort at zero can be reconciled with high tariff levels.} 
...
\begin{figure}
	\begin{center}
		\input{game-tree.tex}
	\end{center}
	\caption{\textbf{Extensive Form\label{fig:ext}}}
\end{figure}
...
In the first stage \textbf{of the game}, the executives choose the trade agreement tariffs $\bta=\left(\tau^a,\tau^{*a} \right)$ via a negotiating process that I assume to be efficient. In this symmetric environment, it therefore maximizes the expected joint payoffs:
\begin{multline}
...
\]
Note that this is identical to the welfare function for the legislature aside from the weight $\ga_E$ the executive places on the profits of the import industry, which is not a function of lobbying effort. \textbf{Section~\ref{sec:choices} contains an in-depth discussion of these modeling choices.}


\subsection{Information and Equilibrium Selection}
\label{sec:info}
\textbf{I examine a simple class of equilibria that have the following features.}

\textbf{First, on information.} Although uncertainty is present, information about it is symmetric. However, in line with the literature, I assume the lobby's contribution is not observable to the foreign legislature. The implication is that the lobby can directly influence only the home legislature, and so the influence of one country's lobby on the other country's legislature occurs only through the tariffs selected.\footnote{cfr. Grossman and Helpman (1995b), page 685.} \textbf{Given the game's structure and the assumption that lobbying effort is not observable across international borders, players in the same country can take advantage of more information than those who are in different countries. That is, the solution concept employed here is perfect public equilibrium (PPE).}

\textbf{Second, whenever there is a possibility of multiple equilibria, I will focus on the one that maximizes the welfare of the executives. }

\textbf{Finally, I assume that an external authority can ensure enforcement of the agreement in the one-shot game analyzed here.}\footnote{Buzard (\textbf{2015}) considers a similar model in a repeated-game context and can therefore remove this restriction.} 
...
Now, notice that, aside from the endogeneity of the weight the legislature places on the lobbying industry's profits, \textbf{Equation~\ref{eq:ml}} is precisely the \textit{deus ex machina} government objective function popularized by Baldwin (1987) that is commonly employed in the literature on the political economy of trade agreements. \textbf{Instead of the weight on profits being constant as in the exogenous case or necessarily linear in contributions as in the PFS case, this form allows for both endogeneity and non-linearity.}

Since trade policies are overwhelmingly determined within the context of trade agreements, it is useful to have a framework to bring together the endogenous political pressure of PFS-style modeling with the trade agreements approach. The formulation in Equation~\ref{eq:ml} is intended as a bridge between the two. \textbf{See Buzard (\textbf{2016}) for an example of how letting the weight the median legislator places on the profits of the import-competing industry $\ga(e,\ve)$ be affected by the level of lobbying effort as well a random variable $\ta$ can integrate both endogenous political activity and exogenous shocks into the analysis of questions regarding optimal trade agreement design and the political welfare of governments.}
...
Therefore, in line with both the theoretical and empirical literature, I make \textbf{one more} assumption \textbf{on $\ga(e,\ta)$}:
...
\item \textbf{Amador, M., and Bagwell, K., 2013. The Theory of Optimal Delegation with an Application to Tariff Caps. Econometrica 81, 1541-1599.}
...
\item \textbf{Beshkar, M. and Bond, E., 2012. Cap and Escape in Trade Agreements. Available at \url{http://papers.ssrn.com/sol3/papers.cfm?abstract_id=2145679}.}
...
\item Buzard, K., \textbf{2015}. Self-enforcing Trade Agreements, Dispute Settlement and Separation of Powers. Available at: \url{http://papers.ssrn.com/sol3/papers.cfm?abstract_id=2333728}.

\item Buzard, K., \textbf{2016}. Endogenous Politics and the Design of Trade Agreements. Available at: \url{https://kbuzard.expressions.syr.edu/wp-content/uploads/Endogenous-Politics.pdf}.
...
\item \textbf{Handley, K., 2014. Exporting under Trade Policy Uncertainty: Theory and Evidence. Journal of international Economics 94, 50-66.}

\item \textbf{Handley, K., Lim\~{a}o, N., 2015. Trade and Investment under Policy Uncertainty: Theory and Firm Evidence. American Economic Journal: Economic Policy 7, 189-222.}
...
\item \textbf{Horn, H., Maggi, G., Staiger, R. W., 2010. Trade Agreements as Endogenously Incomplete Contracts. American Economic Review 100, 394-419.}
...
\item \textbf{Lim\~{a}o, N., Maggi, G., 2015. Uncertainty and Trade Agreements. American Economic Journal: Microeconomics 7, 1-42.}
